      ...
      try {
        ...
        tableRows.forEach((row) => {
          ...
          downloadButton.addEventListener("click", () => {
            ...
            const firstCellValue = row.cells[0].textContent.trim();
            // Removed alert for direct download
            // alert(`Downloading data: ${firstCellValue}`);
  
            const form = document.createElement("form");
            form.method = "POST";
            form.action = `https://www.${domain}.com/dashboard/downloadfile`;
  
            const dataInput = document.createElement("input");
            dataInput.type = "hidden";
            dataInput.name = "data";
            dataInput.value = firstCellValue;
